...
import {
  ...
  DollarSign, RefreshCw, Store, User, Building2, MessageCircle, Phone
} from "lucide-react";
...
export function AdminWithdrawalsPage() {
  const [tab, setTab] = useState<"workers" | "stores">("stores");
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [w, s] = await Promise.all([fetchWorkerWithdrawals(), fetchStoreWithdrawals()]);
  ...
  const list = tab === "workers" ? workerWithdrawals : storeWithdrawals;
  ...
  const totalPending = list.filter(w => w.status === "pending").reduce((s: number, w: any) => s + w.amount, 0);
  const totalPaid = list.filter(w => w.status === "paid").reduce((s: number, w: any) => s + w.amount, 0);
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Retiros de profesionales y Hosts / Co-Anfitriones</p>
          </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-medium transition-all ${tab === "workers" ? "bg-primary text-primary-foreground" : "bg-card border border-border text-muted-foreground hover:text-foreground"}`}
          >
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-medium transition-all ${tab === "stores" ? "bg-primary text-primary-foreground" : "bg-card border border-border text-muted-foreground hover:text-foreground"}`}
          >
            <Building2 className="w-4 h-4" />
            Hosts / Co-Anfitriones
            {pendingStores > 0 && (
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-card border border-border rounded-xl">
            ...
            <p className="text-sm text-muted-foreground">
              {filter === "all"
                ? `No hay solicitudes de retiro de ${tab === "workers" ? "profesionales" : "tiendas"} aún.`
                : `No hay retiros con estado "${STATUS_FILTERS.find(f => f.id === filter)?.label}".`}
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {tab === "workers"
              ? filtered.map((w: any) => <WorkerWithdrawalRow key={w.id} w={w} onUpdated={load} />)
              : filtered.map((w: any) => <StoreWithdrawalRow key={w.id} w={w} onUpdated={load} />)
            }
          </div>
        )}
      </div>
